...
import { SibebarLists } from "../utils/DummyData/data";
import {
  Box,
  Group,
  NavLink,
    ...
    <section className="flex flex-col">
      {SibebarLists.map((list) => (
        <Box key={list.id} className="overflow-hidden rounded-md">
          <NavLink
            active={list.id === activeStatus}
            variant="filled"
            label={
              <Link href={`${list.path}`}>
                <Group
                  spacing={smScreen ? "xs" : "sm"}
                  ...
                  onClick={() => setActiveStatus(list.id)}
                >
                  {list.leftIcon}
                  <Text className="sm:text-[0.57rem] md:text-[0.6rem] lg:text-[0.7rem] xl:text-sm 2xl:text-base ">
                    {list.name}
                  </Text>
                </Group>
              </Link>
          ...
          >
            {list.nested !== undefined &&
              list.nested.map((nestedLink) => (
                <div
                  key={nestedLink.id}
                  className=" border-gray-300 rounded-md overflow-hidden"
                >
                  <Link href={nestedLink.path} className="">
                    <NavLink
                      label={
                        <Text className="text-[0.6rem] lg:text-[0.7rem] xl:text-sm 2xl:text-base ">
                          {nestedLink.name}
                        </Text>
                      }
